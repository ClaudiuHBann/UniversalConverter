...
public class RadixUCUnitTest : BaseUCUnitTest
{
    ...
    public static IEnumerable<object[]> RadixInput { get; } = [
        ...
        [false, new List<string>(), new RadixRequest(RadixRequestValid) { To = "XXX" }],
        [false, new List<string>(), new RadixRequest(RadixRequestValid) { From = "XXX" }],
        [false, new List<string>(), new RadixRequest(RadixRequestValid) { From = "XXX", To = "XXX" }], // those values are not valid but they can be skipped because they are the same
